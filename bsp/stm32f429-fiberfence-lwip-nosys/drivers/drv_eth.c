...
void MX_ETH_Init(void)
{
	uint8_t MACAddr[6] ;
	uint32_t sn0;
	sn0=*(uint32_t*)(0x1FFF7A10);

  heth.Instance = ETH;
  heth.Init.AutoNegotiation = ETH_AUTONEGOTIATION_ENABLE;
	//heth.Init.MACAddr = (rt_uint8_t*)&stm32_eth_device.dev_addr[0];
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    {
        HAL_ETH_ReadPHYRegister(&heth, PHY_REG_INTERRUPT, &reg_value);
        
        if (reg_value & PHY_INT_LINK_CHANGE)
        {
            do
            {
                HAL_ETH_ReadPHYRegister(&heth, PHY_REG_BMSR, &reg_value);
...
static void phy_register_write(rt_uint16_t reg, rt_uint32_t value)
{
    ...
    HAL_ETH_WritePHYRegister(&heth, reg, value);
...
rt_err_t eth_phy_init(void)
{
    ...
    HAL_ETH_WritePHYRegister(&heth, PHY_REG_BMCR, PHY_RESET);
    
    for (i = 0x10000; i > 0; i--)
    {
        HAL_ETH_ReadPHYRegister(&heth, PHY_REG_BMCR, &reg_value);
    ...
    HAL_ETH_WritePHYRegister(&heth, PHY_REG_BMCR, PHY_AUTONEGOTIATION);
    for (j = 0x10000; j > 0; j--)
    {
        HAL_ETH_ReadPHYRegister(&heth, PHY_REG_BMSR, &reg_value);
    ...
    for (k = 0x10000; k > 0; k--)
    {
        ...
        if (reg_value & PHY_LINKED_STATUS)
        {
            ...
            HAL_ETH_ReadPHYRegister(&heth, 0, &reg_value);
    ...
    HAL_ETH_WritePHYRegister(&heth, PHY_REG_INTERRUPT, PHY_INT_LINK_MASK);
...
//    }
    ...
    //eth_link_exit_config();
		HAL_ETH_WritePHYRegister(&heth, 0, 0x3100);
...
//    stm32_eth_device.dev_addr[5] = *(rt_uint8_t*)(UID_BASE+0);
	  stm32_eth_device.dev_addr[3] = 0x53;
    stm32_eth_device.dev_addr[4] = 0x00;
    stm32_eth_device.dev_addr[5] = 0x24;
